...
const Coins = () => {
	...
	if (error) {
		return (
			...
			<div className ="errorMessage text-is-danger"> {"Error While Fetching Coins"}</div >
			</div>
		)
	}
...
<div className="column mb-8 has-text-centered  is-3 "key={i.name}>
                <div className="box">
                    <span className="is-flex is-align-items-center is-justify-content-center mb-8 mx-auto has-background-success" style="width: 56px; height: 56px; border-radius: 100%;">
					<img src={i.image} alt={"Img"} />	
					</span>
					<h2>{i.rank}</h2>
